...
def _parse_args(args: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--cert",
        default=os.environ.get("SYNC_CERT", None),
        help="Path to server's PEM cert"
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    with SyncClient(base_url=args.server, secret=args.secret, verify=verify) as client:
        ...
        if diff.to_upload or diff.to_delete:
            print(f"[client] Catch-up: {len(diff.to_upload)} upload(s), {len(diff.to_delete)} delete(s)")
        else:
        ...
        for relative_path in diff.to_upload:
            ...
            manifest.set(relative_path, sha256=compute_sha256(abs_path), mtime=abs_path.stat().st_mtime)
